...
public class Kernel
{
	...
	case INTERRUPT_SOFTWARE: // System calls
	    ...
	    int currentThreadTid = myTcb.getTid();
	    int childThread = waitQueue.enqueueAndSleep(currentThreadTid);

		return childThread; // return a child thread id who woke me up
	    case EXIT:
		...
		myTcb = scheduler.getMyTcb();
		int currentThreadPid = myTcb.getPid();
		currentThreadTid = myTcb.getTid();
		waitQueue.dequeueAndWakeup(currentThreadPid,currentThreadTid);
